...
async def main():
    ...
    try:
        ...
        logging.debug(f"monitor_task failed, (reason: {exc})")
        await github_helper.update_status(
            state=State.FAILURE,
            target_url=target_url,
        ...
        for test_case in test_cases:
            logging.debug(f"{test_case.name=} {test_name=}")
            if test_case.name == test_name:
                test_case.task = Task(task, mode=TaskMode.POLLING)
                await testcase_result_checker_and_update_status(test_case, github_helper)
